...
const createProjectMetadata = (getMetadata: (projectName: string, ctx: Record<string, unknown>) => Partial<Metadata>) => async (context, parent) => {
  ...
  return {
    ...defaultMetadata,
    ...metadata,
    title: metadata.title ? `${metadata.title} | po8klasie ${projectName}` : defaultMetadata.title
  };
};

export const createSchoolMetadata = () => createProjectMetadata(
  async (projectName, {params: {rspo}}) => {
