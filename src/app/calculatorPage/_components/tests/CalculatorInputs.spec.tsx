...
beforeEach(() => {
    vi.mocked(reactHookForm.useFormContext).mockImplementation(() => {
        const [value, setValue] = useState('')
        const ref = useRef({
            setValue: vi.fn().mockImplementation((_, v) => setValue(v)),
            setError: vi.fn(),
        })
...
describe('_components/calculator/CalculatorInputs', () => {
    describe('GradeInput', () => {
        ...
        it(`calls setValue with arg null on Backspace`, () => {
            ...
            const input = wrapper.getByTestId('input')
            fireEvent.keyDown(input, { key: 'Backspace' })
            expect(useFormContextMockedFunctions.setValue).toHaveBeenCalledWith(
                'foo',
                null
            )
        })
        ...
        it(`does not call setValue when key different than 1..6, Delete, Backspace was pressed`, () => {
            ...
            fireEvent.keyDown(input, { key: 'ArrowLeft' })
            expect(
                useFormContextMockedFunctions.setValue
        ...
        it(`calls setError when invalid value was entered`, () => {
            ...
            expect(useFormContextMockedFunctions.setError).toHaveBeenCalledWith(
                'bar',
                {
        ...
        possibleGrades.forEach((grade) => {
            it(`calls setValue when valid value (${grade}) was entered`, () => {
                ...
                expect(
                    useFormContextMockedFunctions.setValue
                ).toHaveBeenCalledWith('bar', grade)
            })
            ...
            it(`does not call setError when valid value (${grade}) was entered`, () => {
                ...
                fireEvent.change(input, { target: { value: grade } })
                expect(
                    useFormContextMockedFunctions.setError
                ).not.toHaveBeenCalled()
            })
        })
    })
