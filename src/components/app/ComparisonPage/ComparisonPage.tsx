import { SchoolCardPlaceholder } from "../SchoolCard";
import { nanoid } from "nanoid";
import useComparisonInstitutionsQuery from "./hooks/useComparisonInstitutions";
...
const getClassesMaxNo = (data: ComparisonInstitutionI[] | undefined): number => {
  if(!data)
    return 0;
  const classesNoPerInstitution = data.map(
    institution => institution.comparison.classes.length
  );
