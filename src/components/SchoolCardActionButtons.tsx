...
const SchoolCardAddToFavoritesActionButton: FC<SchoolCardAddToFavoritesAction> = ({
  isFavorite,
  onClick,
}) => (
  <button onClick={onClick} title={isFavorite ? 'Usuń z ulubionych' : 'Dodaj do ulubionych'}>
    <FiStar className={[actionIconClassName, isFavorite ? 'fill-current' : ''].join(' ')} />
  </button>
);

interface SchoolCardAddToComparisonActionButtonProps {
  isToCompare: boolean;
  onClick: MouseEventHandler;
}

const SchoolCardAddToComparisonActionButton: FC<SchoolCardAddToComparisonActionButtonProps> = ({
  isToCompare,
  onClick,
}) => {
...
const SchoolCardActionButtons: FC<SchoolCardActionButtonsProps> = ({ rspo, className }) => {
  ...
  return (
    <div className={className ?? ''}>
      <SchoolCardAddToFavoritesActionButton
        isFavorite={isFavorite}
        onClick={handleAddToFavoritesClick}
      />
      <SchoolCardAddToComparisonActionButton
