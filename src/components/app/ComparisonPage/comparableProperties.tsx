...
export interface ComparablePropertiesSectionI {
  sectionName: string;
  getBlankItemsNo?: (comparisonItems: ComparisonItemsSchemaI, propertiesMaxLengths: PropertiesMaxLengthsI) => number;
...
const renderComparisonListItemsFromArray = <T extends string>(arr: ComparisonItemI<T>[]) => (
  <>
    {arr.map(({ value, comparisonResult }) => (
      <ComparisonListItem key={value} state={comparisonResult}>{value}</ComparisonListItem>
    ))}
  </>
);
...
const comparableProperties: ComparablePropertiesSectionI[] = [
  {
    ...
    renderComparisonListContents: ({ isPublic, city }) => (
      <>
        ...
        <ComparisonListItem state={city.comparisonResult}>
          {city.value}
        </ComparisonListItem>
      </>
    )
